    ...
    if (syncGamesBtn) {
        syncGamesBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch('/api/get-boardgames', { method: 'POST' }); // 注意：API端點是 get-boardgames，方法是 POST
                const result = await response.json();
                if (!response.ok) {
                    throw new Error(result.details || '同步失敗');
                }
                alert(result.message || '同步成功！');
                await fetchAllGames(); // 同步後重新載入列表
            } catch (error) {
                alert(`錯誤：${error.message}`);
            } finally {
        ...
        if (createRentalForm) {
            createRentalForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if(rentedGame) rentedGame.for_rent_stock = Number(rentedGame.for_rent_stock) - 1;
                    applyGameFiltersAndRender();
