...
function renderUserList(users) {
    ...
    users.forEach(user => {
        ...
        // 【修改 1】檢查是否需要顯示升級標示
        const needsPerk = user.level > (user.perk_claimed_level || 0);
        const levelDisplay = needsPerk ? `${user.level} ⭐` : user.level; // 加星星 ⭐
        ...
        row.innerHTML = `
            ...
            <td>${levelDisplay}</td> {/* 修改：顯示帶星號的等級 */}
        ...
        // 【修改 2】如果需要福利，整列背景稍微不同 (可選)
        if (needsPerk) {
            row.style.backgroundColor = 'rgba(255, 255, 0, 0.1)'; // 淡黃色背景
        }
    });
}
...
async function handleEditUserFormSubmit(event) {
    ...
    try {
        ...
        // 更新快取並重新渲染
        await init(); // 直接呼叫 init 重新獲取最新資料
        
        handleUserSearch(); // 重新渲染列表
...
function renderUserDetails(data) {
    ...
    // 【新增】檢查是否需要顯示福利提示
    const needsPerk = profile.level > (profile.perk_claimed_level || 0);
    const perkAlertHTML = needsPerk
        ? `<div style="padding: 10px; background-color: rgba(255, 193, 7, 0.2); border: 1px solid var(--warning-color); border-radius: 6px; margin-bottom: 1rem; text-align: center;">
             <p style="margin: 0; font-weight: bold; color: var(--warning-color);">⚠️ 升級福利待領取 (LV ${profile.level})！</p>
           </div>`
        : '';
    // 【新增】福利確認按鈕 HTML
    const claimPerkButtonHTML = needsPerk
        ? `<button id="claim-perk-btn" class="action-btn" data-userid="${profile.user_id}" style="background-color: var(--success-color); margin-top: 5px;">✅ 確認已給予福利</button>`
        : '';
    ...
    contentContainer.innerHTML = `
        <div class="details-grid">
            ...
            <div class="profile-details">
                ${perkAlertHTML} {/* 加入福利提示 */}
                ${claimPerkButtonHTML} {/* 加入福利按鈕 */}
...
// 【修改】setupEventListeners 加入福利按鈕的監聽
function setupEventListeners() {
    ...
    // 【新增】監聽 User Details Modal 內的點擊事件 (使用事件委派)
    const userDetailsModal = document.getElementById('user-details-modal');
    if (userDetailsModal) {
        userDetailsModal.addEventListener('click', async (event) => {
            if (event.target.id === 'claim-perk-btn') {
                const button = event.target;
                const userId = button.dataset.userid;
                if (!userId) return;

                const confirmed = await ui.confirm('您確定已經給予該顧客升級福利了嗎？');
                if (!confirmed) return;
                ...
                try {
                    ...
                    // 刷新 Modal 內容 或 重新開啟 Modal
                    await init(); // 重新獲取最新資料
                    openUserDetailsModal(userId); // 重新渲染 Modal
                } catch (error) {
                    ui.toast.error(`更新失敗: ${error.message}`);
                    button.textContent = '✅ 確認已給予福利';
                    button.disabled = false;
                }
            }
        });
